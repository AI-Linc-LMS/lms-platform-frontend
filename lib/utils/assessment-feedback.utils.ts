 ...
 */
export function buildAssessmentFeedbackPoints(data: AssessmentResult): string[] {
  ...
  const pr = stats.placement_readiness;
  const un = stats.total_questions - stats.attempted_questions;
  const pctTime = stats.percentage_time_taken;
  ...
  } else if (Number.isFinite(pctTime) && pctTime < 40 && stats.attempted_questions >= stats.total_questions) {
    points.push(
