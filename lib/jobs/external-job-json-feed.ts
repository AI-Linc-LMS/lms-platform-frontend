...
const ENRICHED_PATH = "/jobs/external-jobs-feed.enriched.json";
const BASE_PATH = "/jobs/external-jobs-feed.json";

async function tryFetchJson(path: string): Promise<ExternalJobJsonRecord[] | null> {
  const res = await fetch(path, { cache: "force-cache" });
  if (!res.ok) return null;
  const data = (await res.json()) as unknown;
  return Array.isArray(data) ? (data as ExternalJobJsonRecord[]) : null;
}

export async function fetchAndMapExternalJsonJobs(): Promise<JobV2[]> {
  const data = (await tryFetchJson(ENRICHED_PATH)) ?? (await tryFetchJson(BASE_PATH));
  if (!data) {
    if (process.env.NODE_ENV === "development") console.warn("[external-job-json] Failed to fetch", ENRICHED_PATH, "and", BASE_PATH);
    replaceExternalJsonFeedJobs([]);
    return [];
  }
  const mapped = mapExternalJsonRecordsToJobV2List(data);
  const withFav = syncExternalJsonJobFavoriteFlags(mapped);
