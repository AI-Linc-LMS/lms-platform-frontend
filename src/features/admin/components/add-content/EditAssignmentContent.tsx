...
import { updateSubmoduleContent, getSubmoduleContentById, AssignmentContentUpdateData } from "../../../../services/admin/courseApis";
...
const EditAssignmentContent: React.FC<EditAssignmentContentProps> = ({
  ...
  useEffect(() => {
    if (assignmentData) {
      ...
      setMarks(contentDetails.marks?.toString() || assignmentData.marks?.toString() || "");
      setDescription(contentDetails.description || assignmentData.description || "");
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: Error) => {
      ...
      showError("Update Failed", error.message || "Failed to update assignment content");
    },
  });
  ...
  const handleSave = () => {
    ...
    console.log("Form data:", { title: title.trim(), marks: marksNumber, description: description.trim(), due_date: dueDate });
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-600">Update the assignment information</p>
      </div>
