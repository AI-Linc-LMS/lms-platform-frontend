...
const tabList = [
    ...
    { key: 'articles', label: 'Articles', icon: ArticleIcon },
    { key: 'problems', label: 'Problems', icon: ProblemIcon },
    { key: 'quiz', label: 'Quiz', icon: QuizIcon },
    { key: 'subjective', label: 'Subjective', icon: AssignmentIcon },
    { key: 'development', label: 'Development', icon: DevelopmentIcon },
];

const BottomSheet: React.FC<BottomSheetProps> = ({ open, onClose, children, setActiveTab,activeTab }) => {

    if (!open) return null;

    return (
        <div className="fixed inset-0 z-[1112] flex items-end justify-center w-full bg-black/30">
            <div className={`bg-white w-full rounded-t-2xl p-6 shadow-lg relative animate-slideUp`}>
                    ...
                    {tabList.map(tab => (
                        <button
                            ...
                            className={`flex items-center gap-1 px-4 py-1 rounded-md border text-xs transition-colors text-[#255C79] bg-[#D7EFF6] 
                                ${activeTab === tab.key ? 'border-[#255C79]' : 'border-blue-100 bg-[#F4FDFF]'}`}
