            ...
            <div className="flex-1 w-full">
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2 sm:gap-4 text-[#6C757D] mb-4 text-sm sm:text-base">
                ...
                {mockJobDetail.applicationDeadline && (
                  <div className="flex items-center gap-2">
                    ...
                    <span>Apply by {formatDate(mockJobDetail.applicationDeadline)}</span>
                  </div>
                )}
              </div>
              ...
              {mockJobDetail.salary && (
                <div className="flex items-center gap-3 mb-6">
                  <svg className="w-5 h-5 sm:w-6 sm:h-6 text-[#28A745]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" />
                  </svg>
                  <span className="text-[#28A745] font-bold text-lg sm:text-xl">
                    ${mockJobDetail.salary.min.toLocaleString()} - ${mockJobDetail.salary.max.toLocaleString()} / year
                  </span>
                </div>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            ...
            <div className="bg-white rounded-2xl border border-[#DEE2E6] p-4 sm:p-6">
              <h2 className="text-xl sm:text-2xl font-bold text-[#343A40] mb-4">Job Description</h2>
              <div className="prose prose-gray max-w-none">
                {mockJobDetail.description.split('\n\n').map((paragraph, index) => (
                  <p key={index} className="text-[#495057] leading-relaxed mb-4 text-sm sm:text-base">
                    {paragraph}
                  </p>
                ))}
              </div>
            ...
            {/* Requirements */}
            <div className="bg-white rounded-2xl border border-[#DEE2E6] p-4 sm:p-6">
              <h2 className="text-xl sm:text-2xl font-bold text-[#343A40] mb-4">Requirements</h2>
              <ul className="space-y-3">
                {mockJobDetail.requirements.map((req, index) => (
                  <li key={index} className="flex items-start gap-3">
                    <div className="w-2 h-2 bg-[#255C79] rounded-full mt-2 flex-shrink-0"></div>
                    <span className="text-[#495057] text-sm sm:text-base">{req}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            {mockJobDetail.benefits && (
              <div className="bg-white rounded-2xl border border-[#DEE2E6] p-4 sm:p-6">
                <h2 className="text-xl sm:text-2xl font-bold text-[#343A40] mb-4">Benefits & Perks</h2>
                <div className="grid grid-cols-1 gap-3">
                  {mockJobDetail.benefits.map((benefit, index) => (
                    <div key={index} className="flex items-start gap-3">
                      <svg className="w-4 h-4 sm:w-5 sm:h-5 text-[#28A745] mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                      </svg>
                      <span className="text-[#495057] text-sm sm:text-base">{benefit}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-6 lg:sticky lg:top-6 lg:self-start">
            {/* Quick Apply Card */}
            <div className="bg-white rounded-2xl border border-[#DEE2E6] p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl font-bold text-[#343A40] mb-4">Quick Apply</h3>
              ...
              <div className="mt-6 pt-6 border-t border-[#F8F9FA]">
                <div className="text-sm text-[#6C757D] space-y-2">
                  <div className="flex justify-between">
                    <span>Job Type:</span>
                    <span className="font-medium text-[#343A40]">{mockJobDetail.type}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Experience:</span>
                    <span className="font-medium text-[#343A40]">{mockJobDetail.experienceLevel}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Remote:</span>
                    <span className="font-medium text-[#343A40]">{mockJobDetail.remote ? 'Yes' : 'No'}</span>
                  </div>
                  {mockJobDetail.applicationDeadline && (
                    <div className="flex justify-between">
                      <span>Deadline:</span>
                      <span className="font-medium text-[#343A40]">{formatDate(mockJobDetail.applicationDeadline)}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Company Info Card */}
            <div className="bg-white rounded-2xl border border-[#DEE2E6] p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl font-bold text-[#343A40] mb-4">About {mockJobDetail.company}</h3>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h4 className="font-semibold text-[#343A40]">{mockJobDetail.company}</h4>
              ...
              <p className="text-[#495057] text-sm leading-relaxed mb-4">
                ...
                that transform how businesses operate. We're committed to creating a diverse and 
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-[#343A40]">1000-5000 employees</span>
                </div>
                <div className="flex justify-between">
