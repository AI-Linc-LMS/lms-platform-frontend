...
export const useCourseFilters = (courses: Course[] | undefined) => {
  ...
  const filteredCourses = useMemo(() => {
    ...
    return courses.filter((course) => {
      ...
      if (selectedCategories.length > 0 && course.categories) {
        ...
        if (!selectedCategories.some(cat => course.categories?.includes(cat))) {
          return false;
        }
      }
      ...
      if (selectedRatings.length > 0 && course.rating !== undefined) {
        ...
        if (selectedRatings.includes('3_up') && course.rating < 3 && !selectedRatings.includes('4_up')) {
  ...
  }, [courses, searchQuery, selectedCategories, selectedLevels, selectedPrices, selectedRatings]);
