...
import { useFlagshipPayment } from "../../../../../hooks/useRazorpayPayment";
...
const ScholarshipCountdown: React.FC<ScholarshipCountdownProps> = ({
    ...
    useFlagshipPayment({
      onSuccess: (result: PaymentResult) => {
        showToast?.(
          ...
          "Your Flagship seat has been reserved."
        );
  ...
  const handleSeatBookingPayment = () => {
    ...
    showToast?.(
      "loading",
      "Processing Payment",
      "Initiating seat booking payment..."
    );
  ...
  return (
    <div
      className={`w-full bg-gradient-to-r from-yellow-50 to-orange-50 border-l-4 border-yellow-400 p-4 rounded-r-lg shadow-md ${className}`}
    >
      <div className="flex flex-col space-y-3">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-lg sm:text-xl font-bold text-yellow-700">
            ...
            <span className="text-blue-700">Flagship Program</span> and have{" "}
            <span className="text-green-700">won a scholarship</span>
          </h1>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center bg-white rounded-lg p-3 shadow-sm border border-yellow-200 space-y-3 sm:space-y-0 sm:space-x-4">
          <div className="w-full sm:w-auto text-center sm:text-left">
            ...
            <p className="text-sm text-gray-600 mt-2 text-center sm:text-left">
              <strong>Flagship Career Launchpad Course</strong> - Complete
              program with mentorship & job referrals
            </p>
          </div>
          ...
          <div className="w-full sm:w-auto">
            {!isFlagshipCoursePaid && !isFlagshipSeatBooked && !isExpired && (
              <button
                ...
                className="w-full bg-blue-600 text-[var(--font-light)] font-bold py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors duration-200 shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex flex-col items-center justify-center"
              >
                {isSeatBookingProcessing ? (
                  <>
                    <svg
                      className="animate-spin -ml-1 mr-3 h-5 w-5 text-[var(--font-light)] inline"
                  ...
                  <>
                    ...
                    <span className="text-xs font-normal mt-1">
                      Reserve Now, Pay ₹{scholarshipPrice - seatBookingPrice}{" "}
                      Later | 100% Secure Payment
                    </span>
                  </>
                )}
              </button>
                ...
                <React.Fragment key={unit}>
                  <div className="flex flex-col items-center">
                    <span className="text-sm sm:text-lg font-bold text-blue-700 bg-white rounded-lg px-2 sm:px-3 py-1 shadow-md border-2 border-blue-200">
                      {countdown[unit as keyof CountdownState]
                        .toString()
                        .padStart(2, "0")}
                    </span>
        ...
        {!isFlagshipCoursePaid && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-center">
            <p className="text-xs sm:text-sm text-green-700">
              💰 You save <strong>₹{savingsAmount.toLocaleString()}</strong>{" "}
