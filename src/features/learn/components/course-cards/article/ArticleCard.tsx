...
const ArticleCard: React.FC<ArticleCardProps> = ({
  ...
  const parseHtmlContent = (htmlContent: string) => {
    ...
    try {
      ...
      if (htmlContent.includes("<") && htmlContent.includes(">")) {
        ...
        const htmlMatch = processedContent.match(
          /<html[^>]*>([\s\S]*?)<\/html>/i
        );
  ...
  const renderLoadingState = () => {
    ...
    return (
      <div
        className={`${layoutConfig.container?.className || ""} animate-pulse`}
        style={layoutConfig.container?.style}
      >
      ...
      <div className={headerConfig.className || ""} style={headerConfig.style}>
        ...
        {headerConfig.showMarks && (
          <div className="flex flex-col items-end">
            <span className={headerConfig.marksClassName || ""}>
              {marks} Marks
            </span>
          </div>
        )}
        ...
        >
          ...
          {actionsConfig.showIcon && (
            <span className="flex items-center justify-center w-6 h-6 rounded-full">
              <img src={completedIcon} alt="check" className="w-6 h-6" />
            </span>
          )}
        </button>
      </div>
