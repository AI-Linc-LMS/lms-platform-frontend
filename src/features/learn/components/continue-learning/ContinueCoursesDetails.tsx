...
const ContinueCoursesDetails = ({ clientId }: { clientId: number }) => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[1, 2, 3, 4].map((index) => (
          <div
            key={index}
            className="w-full border-[var(--primary-200)] rounded-[16px] md:rounded-[22px] border-[1px] bg-[var(--neutral-50)] p-4 flex flex-col min-h-[280px] md:min-h-[300px] animate-pulse"
          >
  ...
  const courses: CourseData[] = continueCourses
    .slice(0, 4)
    .map((course: ContinueCourse) => ({
      title: course.title,
