...
const SortDropdown = ({ selectedSort, setSelectedSort, className }: SortDropdownProps) => {
      ...
      >
        {sortOptions.map((option) => (
          <MenuItem
            ...
            sx={{
                color: option.value === selectedSort ? '#343A40' : '#495057',
                fontWeight: option.value === selectedSort ? 500 : 400,
            }}
          >
            {option.value === selectedSort && <Check size={16} className="mr-2" />}
            {option.label}
          </MenuItem>
        ))}
      </Menu>
