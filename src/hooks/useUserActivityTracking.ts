    ...
    try {
      ...
      if (lastActivityStr) {
        ...
        if (lastActivity && typeof lastActivity.totalTimeSpent === "number") {
          ...
          console.log(
            `Recovered total time from last activity: ${lastActivity.totalTimeSpent}s`
          );

          // Log if there was an active session at the time of backup
          if (lastActivity.activeSessionDuration) {
            console.log(
              `Backup included active session of: ${lastActivity.activeSessionDuration}s`
            );
          }
      ...
      const sessionBackupStr = localStorage.getItem(
      ...
      if (sessionBackupStr) {
        ...
        if (sessionBackup && typeof sessionBackup.totalTimeSpent === "number") {
          console.log(
            `Recovered total time from session backup: ${sessionBackup.totalTimeSpent}s`
          );
