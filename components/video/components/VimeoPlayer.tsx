    ...
    const handleVimeoMessage = (event: MessageEvent) => {
      ...
      if (data.event === "timeupdate" || data.event === "playProgress") {
        ...
        if (data.data?.seconds && data.data?.duration) {
          ...
          currentTime = data.data.seconds;
          duration = data.data.duration || videoDuration;
        }
      ...
      // Handle "ended" or "finish" event - Vimeo uses "finish" for completion
      if (
        (data.event === "ended" || data.event === "finish") &&
        onComplete &&
        !hasMarkedComplete
      ) {
        ...
        if (videoId) {
          try {
            ...
            const storageKeys = [`video_progress_v1_${videoId}_${title || ""}`];
