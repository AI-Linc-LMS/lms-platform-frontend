    ...
    <PanelCard>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1.25 }}>
        <Stack direction="row" spacing={1.25} alignItems="center">
          ...
          <Box>
            ...
            <Typography sx={{ fontSize: phoneText(0.72), color: "#64748b" }}>this week · top {me.percentile}%</Typography>
          </Box>
        </Stack>
      </Stack>
      ...
      <Stack spacing={0.75}>
        {leaderboard.rows.map((r) => {
          ...
          return (
            ...
            >
              <Box sx={{ width: 22, height: 22, borderRadius: "50%", flexShrink: 0, display: "grid", placeItems: "center", fontSize: phoneText(0.66), fontWeight: 800, color: RANK_FG[r.rank] || "#64748b", bgcolor: RANK_BG[r.rank] || "#f1f5f9" }}>{r.rank}</Box>
      ...
      {leaderboard.aiTip && (
        <Stack direction="row" spacing={0.6} alignItems="flex-start" sx={{ mt: 1.25, p: 1, borderRadius: 2, background: "linear-gradient(135deg, #f5f3ff, #fdf2f8)" }}>
          ...
          <Typography sx={{ fontSize: phoneText(0.74), color: "#6d28d9", fontWeight: 600, lineHeight: 1.45 }}>{leaderboard.aiTip}</Typography>
        </Stack>
      )}
    </PanelCard>
  );
