...
function DecayChart({ spec }: { spec: DecaySpec }) {
  // The chart scales to its card. At 480 units wide in a ~325px phone card, 9-unit labels come
  // out near 6px, so a phone draws a narrower canvas at close to 1:1 with 12px labels. Wider
  // screens keep the original geometry.
  const theme = useTheme();
  const isPhone = useMediaQuery(theme.breakpoints.down("sm"));
  const W = isPhone ? 320 : 480, H = isPhone ? 160 : 150, padL = isPhone ? 36 : 30, padB = isPhone ? 26 : 22, padT = 10, padR = 10;
  const label = isPhone ? 12 : 9;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}>
      ...
      <text x={x(spec.grace / 2)} y={padT + (isPhone ? 16 : 14)} textAnchor="middle" fontSize={label} fontWeight="700" fill="#16a34a">full</text>
      {[spec.base, Math.round(spec.base / 2), 0].map((p) => (
        <text key={p} x={padL - 6} y={y(p) + (isPhone ? 4 : 3)} textAnchor="end" fontSize={label} fill="#94a3b8">{p}</text>
      ))}
      {ticks.map((t) => (
        <text key={t} x={x(t)} y={H - 6} textAnchor="middle" fontSize={label} fill="#94a3b8">{fmtTick(t)}</text>
      ))}
  ...
  return (
    <Box sx={{ ...CARD, p: 2 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="baseline" sx={{ mb: 1 }}>
        ...
        <Typography sx={{ fontSize: phoneText(0.72), color: "#94a3b8" }}>grace {fmtSecs(spec.grace)}, then −{spec.dec} / {spec.iv}s</Typography>
      </Stack>
...
export function PointsSystemContent() {
  ...
  return (
    <Box>
      {/* Hero */}
      <Reveal>
        <Box sx={{ borderRadius: 5, p: { xs: 2.5, md: 3.5 }, mb: 3, color: "white", position: "relative", overflow: "hidden", background: "radial-gradient(110% 130% at 90% -10%, rgba(236,72,153,0.45) 0%, rgba(124,58,237,0.0) 55%), linear-gradient(135deg, var(--module-hero-from, #7c3aed) 0%, var(--module-hero-mid, #a855f7) 55%, var(--module-hero-to, #c026d3) 100%)", boxShadow: "0 24px 60px -30px var(--module-hero-shadow, rgba(124,58,237,0.7))" }}>
          <Stack direction="row" spacing={1.75} alignItems="flex-start">
            ...
            <Box sx={{ minWidth: 0 }}>
              <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                ...
                <Box sx={{ px: 1, py: 0.3, borderRadius: 999, fontSize: phoneText(0.62), fontWeight: 800, color: "#7c3aed", bgcolor: "white" }}>UNIFIED</Box>
              </Stack>
            ...
            {data.activities.map((a) => (
              <Stack key={a.key} direction="row" alignItems="center" spacing={1.25} sx={{ ...CARD, p: 1.75, borderLeft: "3px solid", borderLeftColor: a.accent }}>
                ...
                <Box sx={{ minWidth: 0, flex: 1 }}>
                  ...
                  <Typography sx={{ fontSize: phoneText(0.7), color: "#94a3b8" }}>{a.sub}</Typography>
                </Box>
                <Box sx={{ textAlign: "right", flexShrink: 0 }}>
                  <Typography component="span" sx={{ fontWeight: 900, fontSize: "1.05rem", color: a.accent }}>{a.points}</Typography>
                  <Typography component="span" sx={{ fontSize: phoneText(0.66), color: "#94a3b8", ml: 0.3 }}>{a.unit}</Typography>
                </Box>
              </Stack>
            ))}
          </Box>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 1fr 340px" }, gap: 1.5 }}>
            ...
            <Box sx={{ p: 2.25, borderRadius: 4, color: "#fff", backgroundColor: "#0f0a28", backgroundImage: "linear-gradient(160deg, #1a1442, #0f0a28)", boxShadow: "0 18px 40px -24px rgba(76,29,149,0.6)" }}>
              <Stack direction="row" spacing={0.75} alignItems="center" sx={{ mb: 1 }}>
                ...
                <Typography sx={{ fontSize: phoneText(0.66), fontWeight: 800, letterSpacing: 0.8, color: "rgba(255,255,255,0.6)" }}>THE FORMULA</Typography>
              </Stack>
              ...
              <Typography sx={{ fontSize: phoneText(0.74), color: "rgba(255,255,255,0.65)", mt: 1.5, lineHeight: 1.55 }}>{data.formulaNote}</Typography>
            </Box>
          </Box>
        </Box>
      </Reveal>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 1fr" }, gap: 2, mb: 4 }}>
          <Box sx={{ ...CARD, p: 2.25, "&:hover": undefined }}>
            ...
            <Stack direction="row" spacing={0.6} alignItems="flex-start" sx={{ mt: 1.5, p: 1.25, borderRadius: 2, bgcolor: "#f5f3ff" }}>
              ...
              <Typography sx={{ fontSize: phoneText(0.74), color: "#6d28d9", fontWeight: 600, lineHeight: 1.45 }}>
      ...
      {/* Worked example */}
      <Reveal>
        <Box sx={{ borderRadius: 5, p: { xs: 2.5, md: 3.5 }, color: "white", position: "relative", overflow: "hidden", background: "radial-gradient(110% 130% at 10% 110%, rgba(236,72,153,0.4) 0%, rgba(124,58,237,0) 55%), linear-gradient(135deg, var(--module-hero-from, #7c3aed) 0%, var(--module-hero-mid, #a855f7) 55%, var(--module-hero-to, #c026d3) 100%)", boxShadow: "0 24px 60px -30px var(--module-hero-shadow, rgba(124,58,237,0.7))" }}>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "300px 1fr" }, gap: { xs: 2.5, md: 4 }, alignItems: "center" }}>
            <Box>
              <Stack direction="row" spacing={0.75} alignItems="center" sx={{ mb: 1 }}>
                ...
                <Typography sx={{ fontSize: phoneText(0.66), fontWeight: 800, letterSpacing: 0.8, color: "rgba(255,255,255,0.75)" }}>WORKED EXAMPLE · ONE WEEK</Typography>
              </Stack>
              ...
              <Stack spacing={0.5}>
                {data.workedExample.rows.map((r) => (
                  <Stack key={r.label} direction="row" alignItems="center" spacing={1.5} sx={{ py: 0.85, borderBottom: "1px solid rgba(255,255,255,0.12)" }}>
                    ...
                    <Box sx={{ px: 0.85, py: 0.2, borderRadius: 999, fontSize: phoneText(0.62), fontWeight: 800, color: "white", bgcolor: r.late ? "rgba(239,68,68,0.4)" : "rgba(255,255,255,0.18)", minWidth: 64, textAlign: "center" }}>
